from core.models import MovimientoPedido, Pedido
from core.serializers.pedido import (
    ChoiceSerializer,
    MovimientoPedidoCreateSerializer,
...
class MisPedidosAPIView(APIView):
    ...
    @swagger_auto_schema(
        operation_description="Obtener los pedidos realizados por un cliente"
    )
...
class PedidoCreateAPIView(APIView):
    ...
    def post(self, request):
        serializer = PedidoCrearSerializer(
            data=request.data, context={"request": request}
        )
        if serializer.is_valid():
            pedido = serializer.save()
            return Response(
                {"id": pedido.id, "mensaje": "Pedido creado correctamente"},
...
class PedidoDetalleAPIView(APIView):
    ...
    def get(self, request, pk):
        try:
            ...
            return Response(
                {"detail": "Pedido no encontrado o no autorizado"},
...
class PedidosRecibidosAPIView(APIView):
    ...
    def get(self, request):
        ...
        pedidos = Pedido.objects.filter(comercio_id__in=comercio_ids).order_by(
            "-creado_en"
        )
            ...
            properties={
                "estado": openapi.Schema(
                    ...
                    description=(
                        ...
                        "(registrado, confirmado, cancelado, entregado)"
                    ),
...
class EstadoPedidoChoicesAPIView(APIView):
    @swagger_auto_schema(
        operation_description="Obtener listado de los estados posibles de un pedido"
    )
    def get(self, request):
