...
from core.models import Producto, Comercio, SemitaImagen, TipoSemita
from core.serializers.producto import ProductoSerializer, SemitaImagenSerializer, TipoSemitaSerializer
...
class MisProductosAPIView(APIView):
    ...
    @swagger_auto_schema(operation_description="Listar los productos de los comercios del usuario autenticado.")
    ...
    @swagger_auto_schema(
        ...
        operation_description="Crear un producto asociado a un comercio del usuario autenticado."
    )
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if comercio.usuario != request.user:
                return Response(
                    {"detail": "No tenés permiso para cargar productos en este comercio."},
...
class ProductoDetalleAPIView(APIView):
    ...
    @swagger_auto_schema(
        ...
        operation_description="Obtener producto por id"
    )
    ...
    @swagger_auto_schema(
        request_body=ProductoSerializer,
        operation_description="Editar parcialmente un producto del usuario autenticado."
    )
...
class SemitaImagenUploadAPIView(APIView):
    ...
    def post(self, request, producto_id):
        ...
        serializer = SemitaImagenSerializer(data=request.data, context={'producto': producto})
...
class SemitaImagenListAPIView(APIView):
    ...
    @swagger_auto_schema(
        operation_description="Listar todas las imágenes de un producto del usuario autenticado.",
        manual_parameters=[
            openapi.Parameter('producto_id', openapi.IN_PATH, type=openapi.TYPE_INTEGER, required=True, description="ID del producto"),
        ],
    ...
    def get(self, request, producto_id):
        ...
        if producto.comercio.usuario != request.user:
            raise PermissionDenied("No tenés permiso para ver las imágenes de este producto.")
...
class SemitaImagenDeleteAPIView(APIView):
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter('producto_id', openapi.IN_PATH, type=openapi.TYPE_INTEGER, required=True),
            openapi.Parameter('imagen_id', openapi.IN_PATH, type=openapi.TYPE_INTEGER, required=True),
        ],
...
class ProductosPorComercioAPIView(APIView):
    ...
    def get(self, request, comercio_id):
        ...
        productos = Producto.objects.filter(comercio=comercio, activo=True, en_stock=True)
        serializer = ProductoSerializer(productos, many=True)
        return Response(serializer.data)
