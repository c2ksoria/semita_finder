from core.models import Comercio, ItemPedido, MovimientoPedido, Pedido, Producto
...
class PedidoSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        pedido = Pedido.objects.create(
            cliente=self.context["request"].user, **validated_data
        )
        ...
        for item_data in items_data:
            ...
            ItemPedido.objects.create(
                pedido=pedido, producto=producto, cantidad=cantidad, precio_item=precio
            )

        MovimientoPedido.objects.create(
            pedido=pedido,
            estado="registrado",
            observacion="Pedido creado automáticamente",
        )
    ...
    def validate_fecha_hora_retiro(self, value):
        if value <= timezone.now():
            raise serializers.ValidationError(
                "La fecha y hora de retiro debe ser futura."
            )
        return value
